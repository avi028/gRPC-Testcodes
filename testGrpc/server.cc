...
struct thread_arg{
  int tid;
  testGrpcAsync::AsyncService *service_;
...
class sayHelloMthod: public baseClass{
  ...
  public :
    sayHelloMthod(testGrpcAsync::AsyncService* service, ServerCompletionQueue* cq,int thread_id)
    ...
    void Proceed() {
      if (status_ == CREATE) {
        status_ = PROCESS;
        service_->Requesthye(&ctx_, &request_, &responder_, cq_, cq_,this);
      } 
      else if (status_ == PROCESS) {
        new sayHelloMthod(service_, cq_,this->tid);
        std::string prefix(" hye ");
        std::string tid_str = std::to_string(this->tid);
        reply_.set_message(tid_str+prefix + request_.message());
        status_ = FINISH;
...
class sayByeMthod: public baseClass{
  private:
    testGrpcAsync::AsyncService *service_;
    ServerCompletionQueue* cq_;
    ServerContext ctx_;
  ...
  public :
    sayByeMthod(testGrpcAsync::AsyncService* service, ServerCompletionQueue* cq,int thread_id)
      : service_(service), cq_(cq), responder_(&ctx_), status_(CREATE), tid(thread_id) {
      Proceed();
    }
    ...
    void Proceed() {
      if (status_ == CREATE) {
        status_ = PROCESS;
        service_->Requestbye(&ctx_, &request_, &responder_, cq_, cq_,this);
      } 
      else if (status_ == PROCESS) {
        new sayByeMthod(service_, cq_,this->tid);
...
void * HandleRpcs_thread(void * thread_arg){
    ...
    new sayByeMthod((arg->service_), arg->cq_.get(),arg->tid);
